...
<style>
    .badge-status{
        ...
        text-transform: capitalize; color:#fff; font-weight:600; display:inline-block; min-width:85px; text-align:center;
    }
    .pending{background:#6c757d}
    .shipping{background:#17a2b8}
    ...
    .td-address{max-width:280px; white-space:nowrap; overflow:hidden; text-overflow:ellipsis}
</style>
        ...
        <div class="sherah-body">
          <div class="sherah-dsinner">
            ...
            <form method="GET" action="{{ route('admin.orders.index') }}" class="mb-3">
              ...
              @if(request('status'))
                <a href="{{ route('admin.orders.index') }}" class="btn btn-default ms-2">Xóa lọc</a>
              @endif
            </form>
                  ...
                  <tbody>
                    @forelse($orders as $order)
                      @php
                        ...
                        $paymentClass = $order->payment_status === 'paid' ? 'payment-paid' : 'payment-unpaid';
                      @endphp
                      <tr>
                        ...
                        <td class="td-address" title="{{ $order->receiver_address }}">{{ $order->receiver_address }}</td>
                        <td class="text-end">{{ number_format($order->shipping_fee,0,',','.') }}đ</td>
                        <td class="text-end text-primary fw-bold">{{ number_format($order->final_amount,0,',','.') }}đ</td>
                        <td class="{{ $paymentClass }}">{{ $order->payment_status }}</td>
                        <td><span class="badge-status {{ $statusClass }}">{{ $order->order_status }}</span></td>
                        <td>{{ optional($order->created_at)->format('d/m/Y H:i') }}</td>
                        <td>
                          <a href="{{ route('admin.orders.show', $order->id) }}" class="btn btn-outline-primary btn-sm">Chi tiết</a>
                        </td>
                      </tr>
                    @empty
                      <tr>
