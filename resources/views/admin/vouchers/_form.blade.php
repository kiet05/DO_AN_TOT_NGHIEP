...
<div class="row g-3">
    ...
    <div class="col-md-3">
        ...
        <select name="type" class="form-select">
            <option value="percent" @selected(old('type', $voucher->type ?? '') == 'percent')>
                Giảm %
            </option>

            <option value="fixed" @selected(old('type', $voucher->type ?? '') == 'fixed')>
                Giảm tiền
            </option>
        </select>
    </div>
    ...
    <div class="col-md-3">
        ...
        <select name="apply_type" id="apply_type" class="form-select">
            <option value="all" @selected(old('apply_type', $voucher->apply_type ?? '') == 'all')>
                Tất cả
            </option>
            <option value="products" @selected(old('apply_type', $voucher->apply_type ?? '') == 'products')>
                Sản phẩm chọn
            </option>
            <option value="categories" @selected(old('apply_type', $voucher->apply_type ?? '') == 'categories')>
                Danh mục chọn
            </option>
        </select>
    </div>
